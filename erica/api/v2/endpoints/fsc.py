...
async def request_fsc(request_fsc_data: FreischaltCodeRequestDto, request: Request):
    """
    Route for requesting a new fsc for the sent id_nr using the job queue.
    :param request: API request object.
    :param request_fsc_data: payload with client identifier and the JSON input data for the request.
    """
    result = get_job_service(RequestType.freischalt_code_request).add_to_queue(
        request_fsc_data.payload, request_fsc_data.client_identifier,
...
async def activate_fsc(activation_fsc_data: FreischaltCodeActivateDto, request: Request):
    """
    Route for requesting activation of an fsc for the sent id_nr using the job queue.
    :param request: API request object.
    :param activation_fsc_data: payload with client identifier and the JSON input data for the activation.
    """
    result = get_job_service(RequestType.freischalt_code_activate).add_to_queue(
        activation_fsc_data.payload, activation_fsc_data.client_identifier,
...
async def revocate_fsc(revocation_fsc_data: FreischaltCodeRevocateDto, request: Request):
    """
    Route for requesting revocation of an fsc for the sent id_nr using the job queue.
    :param request: API request object.
    :param revocation_fsc_data: payload with client identifier and the JSON input data for the revocation.
    """
    result = get_job_service(RequestType.freischalt_code_revocate).add_to_queue(
        revocation_fsc_data.payload, revocation_fsc_data.client_identifier,
