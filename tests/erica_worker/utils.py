...
from erica.erica_worker.request_processing.erica_input.v1.erica_input import EstData, FormDataEst, MetaDataEst, UnlockCodeRequestData, \
    UnlockCodeActivationData, UnlockCodeRevocationData
    
from erica.erica_worker.elster_xml.elster_xml_generator import VERANLAGUNGSJAHR
...
def missing_cert():
    return not os.path.exists('erica/erica_worker/instances/blueprint/cert.pfx')


def missing_pyeric_lib():
    return not (os.path.exists('erica/erica_worker/lib/libericapi.so') or os.path.exists('erica/erica_worker/lib/libericapi.dylib'))


def path_to_session_folder(session_id):
    return os.path.abspath(os.path.join("erica/erica_worker/instances/session_" + session_id))


def get_instances_folder_without_anonymous_files():
    return list(filter(lambda path: not path.startswith('.'),
                       os.listdir(path=os.path.abspath("erica/erica_worker/instances"))))
