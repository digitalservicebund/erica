...
from tests.erica_worker.utils import gen_random_key, missing_cert, missing_pyeric_lib
from erica.erica_worker.pyeric.eric import EricWrapper, EricDruckParameterT, EricVerschluesselungsParameterT, EricResponse, \
    get_eric_wrapper
from erica.erica_worker.pyeric.eric_errors import EricProcessNotSuccessful, EricNullReturnedError, EricGlobalError
from tests.utils import read_text_from_sample

TEST_CERTIFICATE_PATH = 'erica/erica_worker/instances/blueprint/cert.pfx'
...
class TestGetEricWrapper(unittest.TestCase):
    def test_calls_initialise(self):
        with patch('erica.erica_worker.pyeric.eric.EricWrapper.initialise') as init_fun, \
                patch('erica.erica_worker.pyeric.eric.EricWrapper.shutdown'), \
    ...
    def test_calls_shutdown(self):
        with patch('erica.erica_worker.pyeric.eric.EricWrapper.initialise'), \
             patch('erica.erica_worker.pyeric.eric.EricWrapper.shutdown') as shutdown_fun, \
                patch('builtins.open', mock_open()):
            with get_eric_wrapper():
                # Test the context manager
                pass
...
class TestEricInitialise(unittest.TestCase):
    ...
    def test_if_log_path_set_then_set_logpath_and_pluginpath_as_arguments(self):
        with patch('erica.erica_worker.pyeric.eric.c_char_p') as char_pointer, \
                patch('erica.erica_worker.pyeric.eric.os.path.dirname', MagicMock(return_value=self.plugin_path)):
...
class TestValidateAndSend(unittest.TestCase):
    ...
    def test_process_method_called_once_with_print_flag_and_correct_print_path(self):
        ...
        with patch('erica.erica_worker.pyeric.eric.pointer') as pointer, \
                patch('erica.erica_worker.pyeric.eric.tempfile.NamedTemporaryFile', MagicMock(return_value=temporary_file_object)):
...
class TestGetCertHandle(unittest.TestCase):
    ...
    def test_if_get_cert_handle_called_then_get_handle_certificate_called_once_with_correct_arguments(self):
        ...
        c_int_pointer = c_int()
        with patch("erica.erica_worker.pyeric.eric.pointer", MagicMock(return_value=c_int_pointer)):
            self.eric_api_with_mocked_binaries.get_cert_handle()
