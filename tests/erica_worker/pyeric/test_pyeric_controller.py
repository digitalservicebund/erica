...
from erica.erica_worker.elster_xml.bufa_numbers import VALID_BUFA_NUMBERS
from erica.erica_worker.pyeric.eric import EricResponse
from erica.erica_worker.pyeric.eric_errors import EricGlobalValidationError, EricIOError
from erica.erica_worker.pyeric.pyeric_controller import PyericProcessController, EstPyericProcessController, \
...
from tests.erica_worker.utils import missing_cert, missing_pyeric_lib, TEST_EST_VERANLAGUNGSJAHR
...
class TestPyericControllerGetEricResponse(unittest.TestCase):
    ...
    def test_run_pyeric_called_with_eric_wrapper(self):
        ...
        with patch("erica.erica_worker.pyeric.pyeric_controller.PyericProcessController.run_eric") as mock_run_eric, \
                patch("erica.erica_worker.pyeric.pyeric_controller.get_eric_wrapper") as mock_get_eric_wrapper:
...
class TestEstPyericControllerGetEricResponse(unittest.TestCase):
    ...
    def setUp(self):
        with open('tests/erica_worker/elster_xml/sample_with_auth.xml', 'r') as f:
            self.correct_input_xml = f.read()
        with open('tests/erica_worker/elster_xml/unsuccesful_sample.xml', 'r') as f:
    ...
    def test_if_elster_successful_then_return_correct_responses(self):
        ...
        with patch("erica.erica_worker.pyeric.eric.EricWrapper.process", MagicMock(return_value=expected_response)):
...
class TestBelegIdRequestPyericControllerRunPyEric(unittest.TestCase):
    ...
    def test_if_pyeric_initialised_then_call_process_verfahren_with_correct_verfahren(self):
        ...
        with patch('erica.erica_worker.pyeric.pyeric_controller.pointer', MagicMock(return_value=transfer_handle)):
            pyeric_controller.run_eric(mock_eric_wrapper)
