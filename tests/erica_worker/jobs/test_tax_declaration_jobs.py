...
class TestTaxDeclarationJob:
    ...
    def test_perform_job_called_with_correct_parameters(self):
        ...
        with patch("erica.erica_shared.job_service.job_service_factory.get_job_service", MagicMock()) as mock_get_service, \
                patch("erica.erica_worker.jobs.tax_declaration_jobs.perform_job", AsyncMock()) as mock_perform_job:
            send_est(request_id)
    ...
    def test_get_job_service_called_with_correct_param(self):
        ...
        with patch("erica.erica_shared.job_service.job_service_factory.get_job_service", MagicMock()) as mock_get_service, \
                patch("erica.erica_worker.jobs.tax_declaration_jobs.perform_job", AsyncMock()):
            send_est(request_id)
