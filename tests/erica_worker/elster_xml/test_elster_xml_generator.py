...
from erica.erica_worker.elster_xml.elster_xml_generator import _pretty, _add_xml_nutzdaten_header, get_belege_xml, \
...
from erica.erica_worker.elster_xml.elster_xml_tree import ElsterXmlTreeNode
from erica.erica_worker.elster_xml.est_mapping import PersonSpecificFieldId
from erica.erica_worker.pyeric.eric import get_eric_wrapper
from erica.erica_worker.pyeric.eric_errors import EricProcessNotSuccessful
from erica.erica_worker.elster_xml.transfer_header_fields import TransferHeaderFields
from tests.erica_worker.utils import missing_cert, missing_pyeric_lib, use_testmerker_env_set_false, TEST_EST_VERANLAGUNGSJAHR
...
class TestGenerateTransferHeader(unittest.TestCase):
    ...
    def test_generate_transfer_header_calls_run_pyeric(self):
        with patch('erica.erica_worker.pyeric.eric.EricWrapper.create_th',
...
class TestAddSterklFields(unittest.TestCase):
    ...
    def test_calls_person_specific_method_if_person_specific_tree_node(self):
        with patch('erica.erica_worker.elster_xml.elster_xml_generator._add_person_specific_sterkl_fields') as fun_pers_spec:
            with patch('erica.erica_worker.elster_xml.elster_xml_generator.Element') as fun_element:
...
class TestVastRequest(unittest.TestCase):
    ...
    def test_add_vast_antrag_nutzdaten(self):
        xml_top = Element('top')
        with patch('erica.erica_worker.elster_xml.elster_xml_generator.dt.date.today', MagicMock(return_value=datetime.date(_BEANTRAGUNGSJAHR, 1, 1))):
