...
from erica.shared.payload.freischaltcode import FreischaltCodeActivatePayload, FreischaltCodeRevocatePayload
...
from erica.worker.request_processing.erica_input.v1.erica_input import UnlockCodeRequestData, \
...
class TestEstValidationRequestProcess(unittest.TestCase):
    ...
    def test_pyeric_controller_is_initialised_with_correct_arguments(self):
        ...
        with patch('erica.worker.pyeric.pyeric_controller.EstPyericProcessController.__init__',
                   MagicMock(return_value=None)) \
                as pyeric_controller_init, \
                patch('erica.worker.pyeric.pyeric_controller.EstPyericProcessController.get_eric_response'), \
                patch('erica.worker.elster_xml.est_mapping.check_and_generate_entries'), \
                patch(
                    'erica.worker.request_processing.requests_controller.EstValidationRequestController.generate_json'), \
                patch('erica.worker.elster_xml.elster_xml_generator.'
...
class TestEstRequestProcess(unittest.TestCase):
    ...
    def test_check_and_generate_entries_is_called_with_eric_mapped_object(self):
        ...
        with patch('erica.worker.request_processing.eric_mapper', MagicMock(return_value=eric_mapped_object)), \
                patch(
                    'erica.worker.request_processing.requests_controller.est_mapping.check_and_generate_entries') as generate_entries, \
                patch('erica.worker.pyeric.pyeric_controller.EstPyericProcessController.get_eric_response'), \
                patch('erica.worker.request_processing.requests_controller.EstRequestController.generate_json'), \
                patch('erica.worker.elster_xml.elster_xml_generator.generate_full_est_xml'):
    ...
    def test_pyeric_get_eric_response_is_called(self):
        ...
        with patch('erica.worker.pyeric.pyeric_controller.EstPyericProcessController.__init__',
                   MagicMock(return_value=None)), \
                patch(
                    'erica.worker.pyeric.pyeric_controller.EstPyericProcessController.get_eric_response') as pyeric_get_response, \
                patch(
                    'erica.worker.request_processing.requests_controller.est_mapping.check_and_generate_entries'), \
                patch('erica.worker.request_processing.requests_controller.EstRequestController.generate_json'), \
                patch('erica.worker.elster_xml.elster_xml_generator.'
                      'generate_full_est_xml', MagicMock(return_value=xml)):
            est_request.process()
    ...
    def test_if_use_testmerker_env_false_and_test_idnr_then_create_xml_is_called_with_use_testmerker_set_true(self):
        ...
        with patch('erica.worker.elster_xml.elster_xml_generator.generate_full_est_xml') as generate_xml_fun, \
                patch('erica.worker.pyeric.pyeric_controller.EstPyericProcessController.get_eric_response'), \
                patch('erica.worker.request_processing.requests_controller.EstRequestController.generate_json'):
    ...
    def test_if_submission_without_tax_nr_then_generate_vorsatz_without_tax_nr_is_called(self):
        ...
        with patch(
                'erica.worker.request_processing.requests_controller.generate_vorsatz_without_tax_number') as generate_vorsatz_without_tax_number, \
                patch('erica.worker.elster_xml.elster_xml_generator.generate_full_est_xml') as generate_xml_fun, \
                patch('erica.worker.pyeric.pyeric_controller.EstPyericProcessController.get_eric_response'), \
                patch('erica.worker.request_processing.requests_controller.EstRequestController.generate_json'):
            est_request = EstRequestController(correct_est)
            est_request.process()
...
class TestUnlockCodeRequestProcess(unittest.TestCase):
    ...
    def test_pyeric_controller_is_initialised_with_correct_arguments(self):
        ...
        with patch('erica.worker.pyeric.pyeric_controller.UnlockCodeRequestPyericProcessController.__init__',
                   MagicMock(return_value=None)) \
                as pyeric_controller_init, \
                patch(
                    'erica.worker.pyeric.pyeric_controller.UnlockCodeRequestPyericProcessController.get_eric_response'), \
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeRequestController.generate_json'), \
                patch('erica.worker.elster_xml.elster_xml_generator.'
    ...
    def test_if_test_idnr_then_create_xml_is_called_with_use_testmerker_set_true(self):
        with patch(
                'erica.worker.elster_xml.elster_xml_generator.generate_full_vast_request_xml') as generate_xml_fun, \
                patch(
                    'erica.worker.pyeric.pyeric_controller.UnlockCodeRequestPyericProcessController.get_eric_response'), \
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeRequestController.generate_json'):
    ...
    def test_if_processed_called_then_elster_request_id_added_to_cache_list(self):
        ...
        with patch('erica.worker.request_processing.requests_controller.TransferticketRequestController.process', MagicMock(return_value={'elster_request_id': elster_request_id})),\
            patch('erica.worker.request_processing.requests_controller.add_new_request_id_to_cache_list') as add_to_cache_list:
            self.unlock_request_with_valid_input.process()
...
class TestUnlockCodeRequestGenerateFullXml(unittest.TestCase):
    ...
    def test_if_dob_date_given_then_call_generate_full_xml_with_unlock_code_eric_mapping(self):
        ...
        with patch(
                'erica.worker.request_processing.requests_controller.elster_xml_generator.generate_full_vast_request_xml') as generate_full_xml:
...
class TestUnlockCodeRequestGenerateJson(unittest.TestCase):
    ...
    def test_if_id_given_and_include_true_then_return_json_with_correct_info(self):
        ...
        with patch('erica.worker.request_processing.requests_controller.get_antrag_id_from_xml',
...
class TestUnlockCodeActivationProcess(unittest.TestCase):
    ...
    def test_pyeric_controller_is_initialised_with_correct_argument(self):
        ...
        with patch('erica.worker.pyeric.pyeric_controller.UnlockCodeActivationPyericProcessController.__init__',
                   MagicMock(return_value=None)) \
                as pyeric_controller_init, \
                patch(
                    'erica.worker.pyeric.pyeric_controller.UnlockCodeActivationPyericProcessController.get_eric_response'), \
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeActivationRequestController.generate_json'), \
                patch('erica.worker.elster_xml.elster_xml_generator.'
    ...
    def test_pyeric_get_eric_response_is_called(self):
        ...
        with patch('erica.worker.pyeric.pyeric_controller.UnlockCodeActivationPyericProcessController.__init__',
                ...
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeActivationRequestController.generate_json'), \
                patch('erica.worker.elster_xml.elster_xml_generator.generate_full_vast_activation_xml',
    ...
    def test_if_test_idnr_then_create_xml_is_called_with_use_testmerker_set_true(self):
        with patch(
                'erica.worker.elster_xml.elster_xml_generator.generate_full_vast_activation_xml') as generate_xml_fun, \
                patch(
                    'erica.worker.pyeric.pyeric_controller.UnlockCodeActivationPyericProcessController.get_eric_response'), \
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeActivationRequestController.generate_json'):
    ...
    def test_if_idnr_and_request_needs_test_merker_then_create_xml_is_called_with_true(self):
        with patch(
                ...
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeActivationRequestController.generate_json'), \
                patch(
                    'erica.worker.request_processing.requests_controller.request_needs_testmerker', MagicMock(return_value=True)):
    ...
    def test_if_idnr_and_request_needs_test_merker_then_create_xml_is_called_with_false(self):
        with patch(
                ...
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeActivationRequestController.generate_json'), \
                patch(
                    'erica.worker.request_processing.requests_controller.request_needs_testmerker', MagicMock(return_value=False)):
            self.unlock_activation_without_idnr.process()
...
class TestUnlockCodeRevocationProcess(unittest.TestCase):
    ...
    def test_pyeric_controller_is_initialised_with_correct_arguments(self):
        ...
        with patch('erica.worker.pyeric.pyeric_controller.UnlockCodeRevocationPyericProcessController.__init__',
                   MagicMock(return_value=None)) \
                as pyeric_controller_init, \
                patch(
                    'erica.worker.pyeric.pyeric_controller.UnlockCodeRevocationPyericProcessController.get_eric_response'), \
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeRevocationRequestController.generate_json'), \
                patch('erica.worker.elster_xml.elster_xml_generator.generate_full_vast_revocation_xml',
                      MagicMock(return_value=xml)):
    ...
    def test_if_test_idnr_then_create_xml_is_called_with_use_testmerker_set_true(self):
        with patch(
                'erica.worker.elster_xml.elster_xml_generator.generate_full_vast_revocation_xml') as generate_xml_fun, \
                patch(
                    'erica.worker.pyeric.pyeric_controller.UnlockCodeRevocationPyericProcessController.get_eric_response'), \
                patch(
                    'erica.worker.request_processing.requests_controller.UnlockCodeRevocationRequestController.generate_json'):
...
class TestCheckTaxNumberRequestControllerProcess:
    ...
    def test_if_generate_electronic_steuernummer_raises_invalid_bufa_nr_then_return_json_with_is_valid_false(self):
        ...
        with patch('erica.worker.request_processing.requests_controller.generate_electronic_steuernummer', MagicMock(side_effect=InvalidBufaNumberError)):
            result = CheckTaxNumberRequestController(input_data).process()
...
class TestGetBelegeRequestController(unittest.TestCase):
    ...
    def test_get_beleg_ids_calls_correct_pyeric_controller_with_correct_argument(self):
        with patch(
                'erica.worker.request_processing.requests_controller.BelegIdRequestPyericProcessController.__init__',
                MagicMock(return_value=None)) as pyeric_controller_mock, \
                patch(
                    'erica.worker.request_processing.requests_controller.elster_xml_generator.generate_full_vast_beleg_ids_request_xml',
                    MagicMock(return_value=self.request_xml)), \
                patch(
                    'erica.worker.request_processing.requests_controller.BelegIdRequestPyericProcessController.get_eric_response'), \
                patch('erica.worker.request_processing.requests_controller.get_relevant_beleg_ids'):
    ...
    def test_get_beleg_ids_calls_get_eric_response(self):
        with patch(
                ...
                MagicMock(return_value=self.request_xml)), \
                patch(
                    'erica.worker.request_processing.requests_controller.BelegIdRequestPyericProcessController.get_eric_response') as fun_get_eric_response, \
                patch('erica.worker.request_processing.requests_controller.get_relevant_beleg_ids'):
            GetBelegeRequestController(self.input_data)._request_beleg_ids()
    ...
    def test_request_encrypted_belege_calls_correct_pyeric_controller_with_correct_argument(self):
        with patch(
                'erica.worker.request_processing.requests_controller.BelegRequestPyericProcessController.__init__',
                MagicMock(return_value=None)) as pyeric_controller_mock, \
                patch(
                    'erica.worker.request_processing.requests_controller.elster_xml_generator.generate_full_vast_beleg_request_xml',
                    MagicMock(return_value=self.request_xml)), \
                patch(
                    'erica.worker.request_processing.requests_controller.BelegRequestPyericProcessController.get_eric_response'), \
                patch('erica.worker.request_processing.requests_controller.get_elements_text_from_xml'):
    ...
    def test_request_encrypted_belege_calls_get_eric_response(self):
        with patch(
                ...
                MagicMock(return_value=self.request_xml)), \
                patch(
                    'erica.worker.request_processing.requests_controller.BelegRequestPyericProcessController.get_eric_response') as fun_get_eric_response, \
                patch('erica.worker.request_processing.requests_controller.get_elements_text_from_xml'):
            GetBelegeRequestController(self.input_data)._request_encrypted_belege(self.sample_beleg_ids)
            fun_get_eric_response.assert_called_once()
    ...
    def test_request_decrypted_belege_calls_get_decrypted_belege_of_correct_pyeric_controller(self):
        with patch(
                'erica.worker.request_processing.requests_controller.DecryptBelegePyericController.get_decrypted_belege') as fun_get_eric_response, \
                patch('erica.worker.request_processing.requests_controller.get_belege_xml'):
    ...
    def test_request_decrypted_belege_returns_decrypted_belege(self):
        ...
        with patch(
                'erica.worker.request_processing.requests_controller.DecryptBelegePyericController.get_decrypted_belege',
...
class TestGetAddressProcess(unittest.TestCase):
    ...
    def test_calls_get_relevant_beleg_ids_with_correct_arguments(self):
        ...
        with patch(
                ...
                patch(
                    'erica.worker.request_processing.requests_controller.get_relevant_beleg_ids') as fun_get_beleg_ids, \
                patch(
                    'erica.worker.request_processing.requests_controller.GetBelegeRequestController._request_encrypted_belege'), \
                patch(
                    'erica.worker.request_processing.requests_controller.GetBelegeRequestController._request_decrypted_belege'), \
                patch(
                    'erica.worker.request_processing.requests_controller.GetAddressRequestController.generate_json'):
...
class TestGetAddressGenerateJson(unittest.TestCase):
    ...
    def test_if_id_given_and_include_true_then_return_json_with_correct_info(self):
        ...
        pyeric_response = PyericResponse(self.expected_eric_response, self.expected_server_response)
        with patch('erica.worker.request_processing.requests_controller.get_address_from_xml',
