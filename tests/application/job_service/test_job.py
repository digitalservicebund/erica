...
class TestJob:
    ...
    async def test_if_service_raises_error_then_job_raises_error(self):
        ...
        with pytest.raises(EricProcessNotSuccessful):
            await perform_job(entity_id=uuid4(), repository=MagicMock(), service=mock_service, payload_type=MagicMock(), logger=MagicMock())
    ...
    async def test_if_service_raises_error_then_log_error_in_warning_logger(self):
        ...
        with pytest.raises(EricProcessNotSuccessful):
            await perform_job(entity_id=uuid4(), repository=MagicMock(), service=mock_service, payload_type=MagicMock(), logger=logger)
    ...
    async def test_if_service_raises_error_then_update_entity_in_database_with_correct_values(self):
        ...
        with pytest.raises(EricProcessNotSuccessful):
            await perform_job(entity_id=uuid4(), repository=mock_repository, service=mock_service, payload_type=MagicMock(), logger=MagicMock())
    ...
    async def test_if_entity_not_found_then_raise_error(self):
        ...
        with pytest.raises(EntityNotFoundError):
            await perform_job(entity_id=uuid4(), repository=mock_repository, service=AsyncMock(), payload_type=MagicMock(), logger=MagicMock())
    ...
    async def test_if_entity_not_found_then_log_error_in_warning_logger(self):
        ...
        with pytest.raises(EntityNotFoundError):
            await perform_job(entity_id=entity_id, repository=mock_repository, service=AsyncMock(), payload_type=MagicMock(), logger=logger)
    ...
    async def test_if_entity_exists_then_log_start_of_job_in_info_logger(self):
        ...
        await perform_job(entity_id=uuid4(), repository=MagicMock(), service=AsyncMock(), payload_type=MagicMock(), logger=logger)
    ...
    async def test_if_entity_exists_then_call_service_with_parsed_entity_payload_and_include_elster_response_true(self):
        ...
        await perform_job(entity_id=uuid4(), repository=mock_repository, service=service, payload_type=MagicMock(parse_obj=dto_parse_obj), logger=MagicMock())
    ...
    async def test_if_job_ran_successful_then_update_entity_in_database_with_correct_values(self):
        ...
        await perform_job(entity_id=uuid4(), repository=mock_repository, service=service, payload_type=MagicMock(), logger=MagicMock())
