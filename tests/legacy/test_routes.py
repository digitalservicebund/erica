...
from erica.legacy.api.v1.endpoints.unlock_code import request_unlock_code, activate_unlock_code, \
    revoke_unlock_code
...
from tests.worker.utils import create_unlock_request, create_unlock_activation, create_est, \
...
class TestSendGrundsteuer(unittest.TestCase):
    ...
    def test_if_request_correct_then_no_error_and_correct_response(self):
        with open("tests/worker/samples/grundsteuer_sample_input.json") as f:
    ...
    def test_if_request_correct_with_bruchteilsgemeinschaft_then_no_error_and_correct_response(self):
        with open("tests/worker/samples/grundsteuer_sample_input_bruchteilsgemeinschaft.json") as f:
            payload = json.loads(f.read())
            parsed_input = GrundsteuerPayload.parse_obj(payload)
            result = send_grundsteuer(parsed_input)
...
class TestRequestUnlockCode(unittest.TestCase):
    ...
    def test_if_request_correct_and_include_then_no_error_and_correct_response(self):
        ...
        try:
            with patch('erica.worker.pyeric.eric.EricWrapper.process', MagicMock(return_value=EricResponse(
                    0,
